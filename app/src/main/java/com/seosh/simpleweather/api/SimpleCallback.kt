...
abstract class SimpleCallback<T> : Callback<T>{
    ...
    override fun onResponse(call: Call<T>, response: Response<T>) {
        ...
        when(response.code()) {
            200 -> {
                // Log.d("seosh", TAG + "[onResponse] Status Success(200) : ${response.body()}")
                onSuccess(response.body())
            }

            else -> {
                 Log.e("seosh", TAG + "[onResponse] Status Fail(${response.code()})")
    ...
    override fun onFailure(call: Call<T>, t: Throwable) {
        Log.e( "seosh", TAG + "[onResponse] Status Error(${t.message})")
        with(FailData.FAIL) {
